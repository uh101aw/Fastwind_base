...
!
!version 7.5.0 Oct. 2020:   compatible with gfortran
!
!version 7.5.1 Aug. 2021:   old bug removed in OPACITL (extrapolation after
!                           update of freq. grid)   
!
!version 7.5.2 Sept 2021:   extend impact of Stark broadening in sumopal_lte
...
if (start) then
  ...
  call atomdat
! JO June 23: now called at first call from frescal
...
enddo
if(vth(8).ne.vth8) stop ' inconsistent values for vth(8)'
...
200 format(2(i2,1x),f10.5,1x,i2,f10.5,1x,f10.0,1x,f10.5,1x,'precision1')
...
& gamma_stark, xnemin, opa_eff_grid, calcopal1
...
! the previous call to FRESCAL_EXPL_LINES) 
  do j=1,nlam
    if(calcopal(j).ne.calcopal1(j)) stop ' calcopal ne calcopal1 in OPACITL'
  enddo
...
&               nb, nt, nb1, nt1, nlcounter1, nlcounter_cmf, ji, nx
...
! that's the new one
   ...
   if(imax(k)-ilow(k) .gt. 2) then
     nx=lwion1-louter(k)
     ilow1(k)=nint(sum(met_imin(k,louter(k):lwion1-1))/float(nx))
     imax1(k)=nint(sum(met_imax(k,louter(k):lwion1-1))/float(nx))
     if(imax1(k)-ilow1(k).ne.2) &
       stop ' something rotten in ilow1, imax1 (subr. jbar_metals)'
     do ll=louter(k),lwion1-1
       charlow=' '
       charmax=' '
       if(met_imin(k,ll).gt.ilow1(k)) charlow='!'
       if(met_imax(k,ll).lt.imax1(k)) charmax='!'
   
       write(*,2),k,ll,met_imin(k,ll),met_imax(k,ll), &
          ilow1(k),charlow,imax1(k),charmax 
     enddo
   else
     do ll=louter(k),lwion1-1
       charlow=' '
       charmax=' '
       if(met_imin(k,ll).gt.ilow1(k)) &
          stop ' something rotten in ilow1, imax1 (subr. jbar_metals)'       
       if(met_imax(k,ll).lt.imax1(k)) &
          stop ' something rotten in ilow1, imax1 (subr. jbar_metals)'          
       write(*,2),k,ll,met_imin(k,ll),met_imax(k,ll), &
          ilow1(k),charlow,imax1(k),charmax 
     enddo
...
2 format(4(i3,2x),' non-SA transport for ',2(i3,a1,2x))   
...
!           if (j.eq.ilow(k) .or. j.eq.imax(k)) then
         if(imax(k)-ilow(k) .gt. 2) then
           if (j.lt.ilow1(k) .or. j.gt.imax1(k)) then
! only Sobo
...
! in this case, opacities for ions 3 in outer part wrong (4,5 correct)
...
USE nlte_app, ONLY: FPATH, NREC_MET=>NREC, KEL, IEL, XION, VTH
...
OPEN(11,FILE=FPATH//'generalinfo',STATUS='OLD',ERR=100,IOSTAT=IOS)
DO I=1,NREC_MET
   READ(11,*)IREC,KEL(I),IEL(I)
ENDDO
CLOSE(11)

OPEN(13,FILE=FPATH//'atomnl2_meta_v2.0',STATUS='OLD',ERR=101,IOSTAT=IOS)
DO I = 1,NREC_MET
  K = KEL(I)
  J = IEL(I)
  READ(13,*) IDUM,IDUM,IDUM,LEVEL,XION(K,J)
  DO LEV=1,LEVEL
    READ(13,*) IDUM
  ENDDO
ENDDO
...
100  WRITE(*,*)' ERROR WITH generalinfo, IOSTAT=',IOS
     STOP
101  WRITE(*,*)' ERROR WITH atomnl2_meta, IOSTAT=',IOS
     STOP
...
USE nlte_lines, ONLY: fgrid, nlam, calcopal,calcopal1, &
                      wavblue, wavred, wavcon
...
do i=1,ifre
  lam=1.d8/fre(i)
  if(lam.gt.wavblue .and. lam.lt.wavred) then
    nlambox = 1 + log(lam/wavblue)/wavcon
    if(nlambox.lt.1 .or. nlambox.gt.nlam) stop ' error in nlambox (frescal_expl_lines)'
    calcopal(nlambox)=1
  endif
enddo
...
j=0
do i=1,nlam
  if(calcopal1(i).ne.calcopal(i)) then
    if(calcopal1(i).ne.1) stop ' error in calcopal1 (frescal_expl_lines)'
    if(calcopal(i) .ne.0) stop ' error in calcopal (frescal_expl_lines)'
...
print*
write(*,10) ifre_lines,wavblue
print*

if(allocated(fre_lines)) stop ' fre_lines allocated in frescal_expl_lines'
...
do i=1,nlam
  if(calcopal1(i).ne.calcopal(i)) then
    meanlam=0.5*(fgrid(i)+fgrid(i+1))
    nlambox = 1 + log(meanlam/wavblue)/wavcon
    if(nlambox.ne.i) stop ' error in nlambox (frescal_expl_lines)'
    j=j+1
...
10 format(i3,' additional frequencies from explicit lines in the range ',f7.1,' to 910 A') 
