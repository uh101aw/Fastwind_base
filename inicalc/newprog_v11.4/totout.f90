...
&                 VMAX,XMLOSS,XMU,YHE,SUMMASS,SUMABU,ERR
INTEGER(I4B) :: I,ICORR,IFRE,J,K,L,NL,NLEV,NU,ZEFFK,NDEF,KISMAX,IDDEP
...
&                 XNELTE(ND),XNH(ND),ZEFF(KEL),ABUND(NATOM),AWEIGHT(NATOM), &
&                 PNEW(ND)
...
IF(IDDEP.NE.0 .AND. IDDEP.NE.1) STOP ' INPUT MUST BE 0 OR 1!'

NLTEPO  = TRIM(CAT)//'/NLTE_POP'  
LTEPOP  = TRIM(CAT)//'/LTE_POP'  

IF(IDDEP.EQ.0) OUT_TOT = TRIM(CAT)//'/OUT_TOT'  
IF(IDDEP.EQ.1) OUT_TOT = TRIM(CAT)//'/OUT_TOT_with_occnum'  
...
IF(IDDEP.EQ.0) PRINT *,'FILE OUT_TOT IN CATALOG ',TRIM(CAT),' WRITTEN'  
IF(IDDEP.EQ.1) PRINT *,'FILE OUT_TOT_with_occnum IN CATALOG ',TRIM(CAT),' WRITTEN'  
