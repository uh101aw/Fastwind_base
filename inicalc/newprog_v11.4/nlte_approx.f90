...
character*10 :: ver_nlte_app='8.3.0' 
...
& jatom,jatom_full,natom,nrec,kel,occ1old2,occngold2
...
&                   gffnlte, ionmax, optneupdate,lwion1,lam_ly, after_update
...
! standard, but niii most likely not converged 
    ...
    if(error_met.lt.0.01 .and. error_met_mean .lt.-2.5) metconv2=.true.
...
integer(i4b), dimension(7,nd) :: tab1, tab2
...
write(*,5) 
do ll=1,nd 
  write(*,10) ll,(tab1(i,ll),tab2(i,ll),i=1,7)
enddo
print*
...
5  format(' ll       >0       0 ... -1   -1...-1.5   -1.5...-2   -2...-2.5   -2.5...-3       <-3')

10 format(i3,7(1x,'|',1x,i4,'/',i4))
