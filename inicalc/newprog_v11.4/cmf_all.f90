...
if (ram .ne. 'big') stop ' cmf_all works only with big ram'
...
! make sure that frequency grid has not changed
  ...
  if(ifretest.ne.ifre) then
    print*,'old:',ifretest,' new:',ifre
    stop ' IFRE HAS CHANGED IN CMF_COMPLETE (2)!!!!'
  endif  
...
! check for frequencies where alo is zero at ALL depth points
  if(k.gt.1. .and. sum(alo_cmf(:,k)).eq.0.) then
    if(opt_ray) then
! shold not happen (alo = 0 only at certain, but not all depth points),
! exept for first cmf iteration (for a couple of first frequencies,
...
!     treat_single=levlo .eq. 'HE21  '.and. xlamcmf(indmat).gt.303. .and. xlamcmf(indmat).lt.305.
     treat_single=.false.
...
!    treat_single=lablinelo(ii) .eq. 'HE21   ' .and. xlam.gt.303. .and. xlam.lt.305. !.OR. &
     treat_single=.false.
     if (opt_hei_sing) then
...
if(kcmf_start1.ne.kcmf_start) stop ' problems in kcmf_start1'
...
!JO March 2018
    if (1.d8/fre(k).gt. 20. .and. (eddftest.lt.0.31 .or. eddftest .gt. 0.35)) &
&      stop ' inconsistent Edd. factor in xj_smooth'
  enddo
...
! sline -> sl_ng only performed when sl_ng(35,i,4) > 0.
     ...
     print*,' since METCONV1 previously set to T!'
     print*
