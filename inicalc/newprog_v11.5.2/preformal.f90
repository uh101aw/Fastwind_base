...
!   in case, use Balmer-lines from Lemke
     ...
     IF(LLCHAR(1:2).EQ.'H1') THEN
       READ(LLCHAR(3:4),'(I)') LL
       READ(LUCHAR(3:4),'(I)') LU
     ENDIF
...
!
     ...
     IF (KEY(:3).EQ.'HE2' .OR. KEY(:2).EQ.'H1') THEN  
          ...
          XL0 = XLAMB(NNN)  
          NN1 = NPPAL(KEY)  
          PRINT *,'H or HeII LINE: GRIEM USED  ',XL0,' ',KEY,' ',LABL(NLEVU(NNN)) 
          KEY = LABL(NLEVU(NNN))  
          NN2 = NPPAL(KEY)  
          AA = DBLE(NN1)  
          BB = DBLE(NN2)  
     ...
     ELSE  
          ...
          PRINT *,'LINE:',LABL(NLEVL(NNN)),'-',LABL(NLEVU(NNN)),' ',XLAMB(NNN)  
          IF (SRANGE1) THEN
...
do i = 1, nline  
read (iu, * ) nlow (i), nup (i), log_alpha0 (i), log_ne0 (i), log_t0 ( &
...
read(charline(4:5),'(i)') nnl
read(charline(10:11),'(i)') nnu
...
read (iu, * ) ( ( (svcs (i, j, k), k = 0, mp (line) ), &
