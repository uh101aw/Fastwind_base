...
character*5 :: ver_formalsol_add='0.5.1' 
...
&                     vturb,yhe,balmer_lemke,pb_lemke,maxncomp)
!
...
! selected elements, division lwion1
        do  ll=1,lwion1-1
!outside
!JP changed Nov. 2022, to include lines (approximate treatment) outside
!   imin, imax 
          occlowi=occng(irec,low,ll) ! occng = n/g
          occupi=occng(irec,up,ll)
! check "exact" NLTE
          if(j.ge.met_imin(k,ll).and.j.le.met_imax(k,ll)) then
            if(occlowi*occupi.le.0.) then
              print*,index,' ',lam
...
!NOTE: for selected elements, LINE_LIST_MERGED also includes lines between
!      levels that are NOT gs,'m',or 's'. i.e. with occup=0, outside imin,imax
!      when ll lt lwion
...
! either inside imin, imax, or beyond LWION, or lines between gs,'m','s' levels
...
! check "exact" NLTE
          if(j.ge.met_imin(k,ll).and.j.le.met_imax(k,ll)) then
            if(occlowi.le.0.) then
              print*,index,' ',lam
              stop ' selected, up=0, outside: occnum = 0 for imin le j le imax'
            endif
          else  
!approximate solution (outside imin,imax) for selected elements:
!should happen only for ll < lwion-1
...
! either inside imin, imax, or beyond LWION1, or gs,'m' or 's' levels
...
call preformal(modnam,icount_hhe,level,leveu,lineno, &
               nstark,vturb,yhe,balmer_lemke,pb_lemke,.true.)
...
!JO May 2017 for tests, can be skipped later
      etal=opal*sl
      if(etal.le.0.) then
        print*,kel,xlam1(i),l,opal_range(l,i),sl,inverted_range(i),i
