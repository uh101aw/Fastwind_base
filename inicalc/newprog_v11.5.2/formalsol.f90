...
! HERE WAS A BUG (RESOL INSTEAD OF XRESOL(NLIN))
     IF(DELTA.LE.XRESOL(NLIN)) STOP ' RESOL TOO LARGE, MODIFY INPUT'
...
!    ARGUMENT .FALSE. CORRESPONDS TO SRANGE IN PREFORMAL
     CALL PREFORMAL(FILE,NCO,LEVEL(1,NLI),LEVEU(1,NLI),LINENO(1,NLI),NSTARK,VTURB,YHE,BALMER_LEMKE,PB_LEMKE,.FALSE.)
...
! Thus, SCON and OPAC (somewhat different from values above) need to be read in
...
IF(EXI) THEN
  ...
  REWIND 1  
  READ (1) ((SCON(I,J),I=1,ND),J=1,IFRETOT),((OPAC(I,J),I=1,ND),J=1,IFRETOT)
  CLOSE (1)  
ENDIF
...
IF(.NOT.OPTTHICK.AND.MAXVAL(ABS(OPA_EFF_RAT(1:ND,1:IFRE)-1.D0)).GT.1.D-6) THEN
...
! COMPARE CONTINUUM FLUXES FROM HERE AND FROM FLUXCONT
     DIFF=MIN(ABS(LOG10(PROFCON(K))-FCONTLOW), &
&              ABS(LOG10(PROFCON(K))-FCONTUP))
     ...
     DIFF=MIN(ABS(1.-PROFCON(K)/10.**FCONTLOW), &
