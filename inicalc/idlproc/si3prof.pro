pro si3prof,cat,star,ps=ps,vsini=vrot1,vrad=vrad,resol=resol1,vmacro=vmacro, $
...
if nspec ne 8 and nspec ne 14 then begin                        ; extended number of lines
  print,' wrong layout!'
  return
  stop
endif
...
file=cat1+'/OUT.SiIV4116'+extension_comp
...
file=cat1+'/OUT.SiIV4212'+extension_comp
...
if nspec eq 14 then begin                                ;adds 4 more lines to be analysed
...
file=cat1+'/OUT.SiIV4089'+extension_comp
...
file=cat1+'/OUT.SiIV4950'+extension_comp
...
file=cat1+'/OUT.SiIV6667'+extension_comp
rtabprof,x13,y13,file,161,6,3,5,ew
if keyword_set(vrot) or keyword_set(resol1) or keyword_set(vmacro) then begin
    convol,x13,y13,xp,yp,resol=resol,vsini=vrot,vmacro=vmacro
      x13=xp
      y13=yp
    endif
	  
file=cat1+'/OUT.SiIV6701'+extension_comp
rtabprof,x14,y14,file,161,6,3,5,ew
if keyword_set(vrot) or keyword_set(resol1) or keyword_set(vmacro) then begin
    convol,x14,y14,xp,yp,resol=resol,vsini=vrot,vmacro=vmacro
      x14=xp
      y14=yp
   endif
endif
...
!p.multi=[0,2,4]                    ;????
if nspec eq 14 then !p.multi=[0,4,4] ;????

!p.title=star+'Si III 4552'                                             ;makes the title of the first plot
!x.range=[4549.,4557.]                                                ; fixs the lambda scale 
...
endelse
if keyword_set(comp) then oplot,x1,y1,linestyle=2                     ;plots the model profile
if keyword_set(obscomp) then begin
...
endelse
if keyword_set(comp) then oplot,x9,y9,linestyle=2
if keyword_set(obscomp) then begin
  if not keyword_set(color) then begin
    oplot,wobs,pobs,thick=2 
  endif else begin
...
endelse
if keyword_set(comp) then oplot,x10,y10,linestyle=2
if keyword_set(obscomp) then begin
  if not keyword_set(color) then begin
    oplot,wobs,pobs,thick=2 
  endif else begin
...
!p.title=star+'Si IV 4116'
!x.range=[4112.,4120.]
if keyword_set(obscomp) then begin
  halpha,cat+'/OUT.SiIV4116'+extension,vsini=vrot,resol=resol,vmacro=vmacro, $
	compmin=min(y7), $
...
!p.title=star+'Si IV 4212'
!x.range=[4208.,4216.]
if keyword_set(obscomp) then begin
  halpha,cat+'/OUT.SiIV4212'+extension,vsini=vrot,resol=resol,vmacro=vmacro, $
	compmin=min(y8), $
  ...
  endelse
  if keyword_set(comp) then oplot,x8,y8,linestyle=2
if keyword_set(obscomp) then begin
  if not keyword_set(color) then begin
    oplot,wobs,pobs,thick=2 
  endif else begin
...
if nspec eq 14 then begin
!p.title=star+'SiIV4089'
!x.range=[4087.,4091.]
if keyword_set(obscomp) then begin
  halpha,cat+'/OUT.SiIV4089'+extension,vsini=vrot,resol=resol,vmacro=vmacro, $
	compmin=min(y11), $
  ...
  endelse
  if keyword_set(comp) then oplot,x11,y11,linestyle=2
if keyword_set(obscomp) then begin
  if not keyword_set(color) then begin
    oplot,wobs,pobs,thick=2 
  endif else begin
  ...
  !p.title=star+'SiIV4950'
  !x.range=[4948.,4952.]
  if keyword_set(obscomp) then begin
    halpha,cat+'/OUT.SiIV4950'+extension,vsini=vrot,resol=resol,vmacro=vmacro, $
	compmin=min(y12), $
    ...
    endelse
    if keyword_set(comp) then oplot,x12,y12,linestyle=2
if keyword_set(obscomp) then begin
  if not keyword_set(color) then begin
    oplot,wobs,pobs,thick=2 
  endif else begin
...
!p.title=star+'SiIV6667'
  !x.range=[6665.,6669.]
  if keyword_set(obscomp) then begin
  halpha,cat+'/OUT.SiIV6667'+extension,vsini=vrot,resol=resol,vmacro=vmacro, $
	compmin=min(y13), $
	compmax=max(y13),wo=wobs,po=pobs,/xnoself
  endif else begin
  halpha,cat+'/OUT.SiIV6667'+extension,vsini=vrot,resol=resol,vmacro=vmacro, $
	compmin=min(y13), $
	compmax=max(y13),/xnoself
  endelse
   if keyword_set(comp) then oplot,x13,y13,linestyle=2
if keyword_set(obscomp) then begin
  if not keyword_set(color) then begin
    oplot,wobs,pobs,thick=2 
  endif else begin
...
!p.title=star+'SiIV6701'
  !x.range=[6699.,6703.]
  if keyword_set(obscomp) then begin
    halpha,cat+'/OUT.SiIV6701'+extension,vsini=vrot,resol=resol,vmacro=vmacro, $
	compmin=min(y14), $
	compmax=max(y14),wo=wobs,po=pobs,/xnoself
  endif else begin
  halpha,cat+'/OUT.SiIV6701'+extension,vsini=vrot,resol=resol,vmacro=vmacro, $
	compmin=min(y14), $
	compmax=max(y14),/xnoself
   endelse
   if keyword_set(comp) then oplot,x14,y14,linestyle=2
if keyword_set(obscomp) then begin
  if not keyword_set(color) then begin
    oplot,wobs,pobs,thick=2 
  endif else begin
