...
;-      negative values now allowed, accounting for erf(-x) = -erf(x)
...
; MEANINGLESS...
; modified to allow for negative arguments

if (total(abs(erfx) le 1d0) ne N_elements(erfx)) then $
    message, 'The range of the error function is -1 <= erf(x) <= 1!'

b=where(erfx lt 0.d0, count)

if count ne 0 then erfx=abs(erfx)
...
xp=0.70710678118654752440d0 * ((t - num/den) > 0)

if count eq 0 then return, xp
xp(b)=-xp(b)
return,xp
