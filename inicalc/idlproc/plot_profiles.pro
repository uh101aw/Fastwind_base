...
; new version for irplot_new.pro
...
; sort out lband lines (for ir)
  ...
  if not file_test(file) then begin
    print,file,' does not exist'
    !p.title=dat.star+dat.titles(i)+' not calc.'
    !x.range=[0.,1.]
    plot,[0.,1.],[1.,1.],yrange=[0.9,1.1],xs=1,/nodata
    goto,cycle
  endif  
...
;    oplot,x(nf(k-1):nf(k)-1),y(nf(k-1):nf(k)-1),color=col(k),psym=1
  ...
  if not color then begin
    ...
    if dat.star eq 'ZP: ' and li eq 'HALPHA' then begin
      ...
      oplot,lamzp,pobszp,color=colorlamc1
    endif  

    oplot,lam,pobs,color=colorwobs
;    if keyword_set(lamc1) then oplot,lamc1,profc1,color=200
    if keyword_set(lamc1) then oplot,lamc1,profc1,color=colorlamc1
  endelse  
  endif
